...
- **First real page built on `/`** — `HomeView` (previously empty) now ships a
  single-page developer portfolio: `Nav`, `Hero`, `About`, `Projects`,
  `Skills`, `Contact`, `Footer`, all under `src/views/home/` per
  [[component-conventions]] ("next to the feature, not in `components/`").
  Placeholder copy lives in `src/data/mocks/home.ts` — no hardcoded content in
  any component.
- **New client leaf: `HeadingReveal`** (`views/home/heading-reveal.tsx`) —
  wraps the `TextEngine` line-reveal pattern from [[text-engine]]. Extracted
  as its own `"use client"` file because `@react-spring/web` ships without a
  `"use client"` boundary of its own: importing `easings` (or any other named
  export) directly inside a Server Component file drags `React.createContext`
  calls into the server bundle and breaks SSR (`createContext only works in
  Client Components`). Any future Server Component that needs `easings` should
  go through a client leaf the same way — worth a line in [[text-engine]] /
  [[animation-system]] if this bites again.
- **Theme: dark-first, no longer OS-dependent** — `--background`/`--foreground`
  in `globals.css` now resolve to the dark values directly instead of behind
  `prefers-color-scheme`, so the portfolio's look is consistent for every
  visitor. Added a lime `--raw-color-brand-*` ramp (the project's first brand
  palette — the starter ships with none by design, see [[design-system]]),
  `--surface` / `--surface-border` / `--muted-foreground` for card and copy
  hierarchy, and `--error` for form validation feedback (replaces reaching for
  Tailwind's default `red-*` scale, which the token rule disallows).
- **`body` reset changed** — the starter's placeholder centering
  (`height: 100vh` + `flex` centering, meant for the empty `<main>`) is gone;
  a real multi-section page needs normal document flow (`min-height: 100vh`).
